...
| **Image Tracking (Natural Feature)** | ⚗️ Experimental | [MindAR](https://github.com/hiukim/mind-ar-js)                                                       | ✅      | ✅             | Tracks arbitrary reference images (logos/posters) via compiled `.mind` targets. Pluggable frame source for a future WebXR Raw Camera Access path. |
...
| **Face Tracking**                    | ⚗️ Experimental | [MediaPipe Face Landmarker](https://ai.google.dev/edge/mediapipe) / ONNX                             | ✅      | ✅             | Webcam + MediaPipe Tasks Vision (WASM/WebGPU); ONNX custom-model path.                     |
...
| **Voice/Gesture Interaction**        | ⚗️ Experimental | MediaPipe Gestures (hand) / [Web Speech API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API) (voice, planned) | ✅      | ✅             | Hand-gesture input available now; voice input planned.                                    |
